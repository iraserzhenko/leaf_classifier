...
Поле `class` содержит название вида растения и диагноз в формате `Plant__Disease` (или `Plant_healthy`). Поле `probability` — уверенность модели в предсказании (после softmax).
...
| **Confusion Matrix**  | Матрица ошибок для анализа межклассовых ошибок                | —                |
...
- **Визуально похожие симптомы:** например, ранний и поздний фитофтороз томата имеют схожий внешний вид
- **Разбивка:** train/val/test = 70%/15%/15% со стратификацией по классам, random seed = 42

## Моделирование

### Бейзлайн

Простая 3-блоковая CNN (~0.3M параметров):
...
5-блоковая CNN с механизмом **Squeeze-and-Excitation (SE)** внимания после каждого сверточного блока (~4.1M параметров).
...
- CPU: 4 ядра, 8 ГБ RAM, ~50 мс/изображение
- GPU (опционально): NVIDIA T4 (16 ГБ), ~5 мс/изображение
- Throughput: ~20 изображений/с на CPU; ~200 изображений/с на GPU (batch=32)

**Пайплайн инференса:**

1. Загрузка изображения (JPEG/PNG)
2. Resize до 256×256 пикселей
3. ToTensor + нормализация ImageNet stats
4. Forward pass через ONNX Runtime (CPU) или Triton Inference Server
5. Softmax по 38 классам
6. Декодирование top-3 индексов в имена классов
7. Возврат JSON с `top1` и `top3`
